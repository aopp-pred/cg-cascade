...
launch () {
  ...
  if ! [[ -f "run_diag.sh" ]]; then
    err 'error: job script "run_diag.sh" must exist'
    return $E_BADJOB
  fi
  # Launch the job array:
  local run_log="${jobname}.diag.%I.log"
  bsub -J "${jobname}[${array_expr}]%24" \
       -o "${run_log}" -e "${run_log}" < run_diag.sh
  if [[ $? -ne 0 ]]; then
    err "error: submission of run_diag.sh failed, check it is a valid LSF job"
    return $E_BADJOB
  fi
  return 0
}
