...
echo "Running pre-processor for time range: ${start_time} - ${end_time} (lcloud=${lcloud})"
ncl coarsen_cascade_manyt.ncl pindex1=${start_index[0]} tindex1=${start_index[1]} pindex2=${end_index[0]} tindex2=${end_index[1]} lcloud=${lcloud}

# Split the output file into one file per time-step:
output_dir="/group_workspaces/jasmin2/aopp/cg-cascade/cg07/data/scm_in"
...
if [[ ${end_index[0]} -eq 120 ]] && [[ ${end_index[1]} -eq 0 ]]; then
...
for i in $(seq 0 $end_slice); do
    ...
    ncks -d time,${i},${i} "$output_file_ncl" "$output_dir/scm_in.${valid_time}.nc"
done
